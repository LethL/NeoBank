...
│       ├── News.entity.ts                       # например, файл для сущности news
...
│       ├── LoanForm                                 # Например, модуль LoanForm, в котором содержится компонент карточки задачи. В этой папке также должны лежать store, стили, внутренние компоненты(папка components), типы. В общем, все что относится к данному модулю

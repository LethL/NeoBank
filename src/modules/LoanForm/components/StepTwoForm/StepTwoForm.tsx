...
export default function StepTwoForm() {
  ...
  const { loading, handleScoringFormSend } = FormStore;
  ...
  const onChangeEmploymentStatus = (event: React.MouseEvent<HTMLSelectElement>) => {
    setValue('employment.employmentStatus', event.currentTarget.value as string);
  };

  const onChangeEmployerINN = (value: string) => {
    setValue('employment.employerINN', value);
  };

  const onChangeSalary = (value: string) => {
    setValue('employment.salary', value);
  };

  const onChangePosition = (event: React.MouseEvent<HTMLSelectElement>) => {
    setValue('employment.position', event.currentTarget.value as string);
  };

  const onChangeWorkExperienceTotal = (value: string) => {
    setValue('employment.workExperienceTotal', value);
  };

  const onChangeWorkExperienceCurrent = (value: string) => {
    setValue('employment.workExperienceCurrent', value);
  };

  const onSubmit = () => {
    handleSubmit(async (data: ScoringFormEntity) => {
      handleScoringFormSend(data);
      setSended(true);
      reset();
    })();
  };

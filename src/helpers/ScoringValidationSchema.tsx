...
export const ScoringValidationSchema = Yup.object({}).shape({
  ...
  // 'employment.employmentStatus': Yup.string().required('Select one of the options'),
  ...
  // 'salary': Yup.number().required('Enter your salary').min(4, 'Salary must be more 3 digits'),
  // 'position': Yup.string().required('Select one of the options'),
  // 'workExperienceTotal': Yup.number().required('Enter your work experience total').max(2, 'Max 2 digits'),
  // 'employment.workExperienceCurrent': Yup.number()
